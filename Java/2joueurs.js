...
// Créer une fonction qui va renvoyer vrai à l'events listeners lorsque l'ordinateur appuie sur z/s
function keyDownHandlerRed(e) {
    if(e.key == "z") {
        upRedPressed = true;
    }
    else if(e.key == "s") {
...
// Créer une fonction qui va renvoyer faux à l'events listeners lorsque l'ordinateur relache z/s
function keyUpHandlerRed(e) {
    if(e.key == "z") {
        upRedPressed = false;
    }
    else if(e.key == "s") {
        downRedPressed = false;
    }
}
...
function drawScoreComputer() {
    ctx.font = "30px Arial";
    ctx.fillStyle = "red";
    ctx.fillText(scoreComputer, 50, 40);
}
...
function drawScorePlayer() {
    ctx.font = "30px Arial";
    ctx.fillStyle = "blue";
    ctx.fillText(scorePlayer, canvas.width-50, 40);
}
...
function draw() {
    ...
    if ((nbBriquesRestantes == 0)||(ball1Status == "gone" && ball2Status == "gone")) {
        ...
        if (scorePlayer>scoreComputer){
        	// Afficher un message pour annoncer que le joueur bleu a gagné
        	alert("LE JOUEUR BLEU A GAGNÉ !");
        }
        else if (scoreComputer>scorePlayer){
        	// Afficher un message pour annoncer que le joueur rouge a gagné
        	alert("LE JOUEUR ROUGE A GAGNÉ !");
        }
        else{
        	// Afficher un message pour annoncer que c'est égalité
        	alert("C'EST ÉGALITÉ ! REFAITENT UNE PARTIE POUR VOUS DÉPARTAGEZ !");
        }
        // Retourner au menu
        window.history.back();
	...
	// Si la touche du bas est enfoncée, ajouter 8 aux coordonnées Y du paddle bleu
	if(downBluePressed) {
		bluePaddleY += 8;
    ...
    // Si la touche du haut est enfoncée, retirer 8 aux coordonnées Y du paddle bleu
	...
	// Si la touche du bas est enfoncée, ajouter 8 aux coordonnées Y du paddle rouge
	if(downRedPressed) {
		redPaddleY += 8;
    ...
    // Si la touche du haut est enfoncée, retirer 8 aux coordonnées Y du paddle rouge
    else if(upRedPressed) {
		redPaddleY -= 8;
		// Vérifier que le paddle rouge ne sort pas du canvas
		if (redPaddleY < 0){
		    redPaddleY = 0;
...
// Appliquer les deux fonctions qui vont renvoyer vrai aux 'events listeners' lorsque l'ordinateur appuie sur z/s
document.addEventListener("keydown", keyDownHandlerRed, false);
document.addEventListener("keyup", keyUpHandlerRed, false);